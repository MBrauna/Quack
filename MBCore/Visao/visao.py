...
class visao_mbrauna:
    ...
    def __init__(self, p_conexao, p_configuracao):
        try:
            ...
            self.VMBRAUNA_CONEXAO       =   p_conexao
            # Armazena as configurações
            self.VMBRAUNA_CONFIGURACAO  =   p_configuracao
            # Carrega a conexão ao DB para execuções
            ...
            vtmp_erro   =   '[VISAO][INIT][ERRO][' + ecx_dados.tb_lineno + '] - ' + p_erro
    ...
    def func_carrega_cores(self):
        ...
        for curreg in range(0,99):
            try:
                ...
                vtmp_erro   =   '[VISAO][FUNC_CARREGA_CORES][ERRO][' + ecx_dados.tb_lineno + '] - ' + p_erro
                self.proc_salva_arquivo(vtmp_erro)
                sys.exit()
            except ValueError as p_erro:
                # Mais detalhes sobre o erro
    ...
    def proc_carrega_tf(self):
        ...
        for csr_modelo in self.VMBRAUNA_TF:
            ...
            try:
                ...
                vtmp_erro   =   '[VISAO][PROC_CARREGA_TF][ERRO][' + ecx_dados.tb_lineno + '] - ' + p_erro
                self.proc_salva_arquivo(vtmp_erro)
                v_tmp_modelo    =   {
                                        'SUCESSO'       :   False
                                       ,'MENSAGEM'      :   p_erro
    ...
    def executa(self, p_comandos):
        try:
            ...
            vtmp_erro   =   '[VISAO][EXECUTAR][ERRO][' + ecx_dados.tb_lineno + '] - ' + p_erro
            self.proc_salva_arquivo(vtmp_erro)
            sys.exit()
    # Carregamento via câmera ou URL
    ...
    def visao(self, p_texto, p_caminho, p_ponto_corte, p_id_sistema_camera):
        try:
            ...
            vtmp_erro   =   '[VISAO][ERRO][' + ecx_dados.tb_lineno + '] - ' + p_erro
            self.proc_salva_arquivo(vtmp_erro)
            sys.exit()
    ...
    def func_inicia_rec(self, p_imagem, p_texto, p_corte, p_id_sistema_camera):
        try:
            ...
            vtmp_erro   =   '[VISAO][FUNC_INICIA_REC][ERRO][' + ecx_dados.tb_lineno + '] - ' + p_erro
            self.proc_salva_arquivo(vtmp_erro)
    ...
    def proc_salva_log(self, p_imagem, p_id_sistema, p_id_classe, p_dim_sup_dir, p_dim_sup_esq, p_dim_inf_dir, p_dim_inf_esq, p_probabilidade):
        try:
            ...
            if vtmp_registro[0][0] is None:
                ...
                vtmp_registro   =   vtmp_registro[0][0]
            # Busca o último registro para aquela detecção.
            ...
            vtmp_erro   =   '[VISAO][PROC_SALVA_LOG][ERRO][' + ecx_dados.tb_lineno + '] - ' + p_erro
            self.proc_salva_arquivo(vtmp_erro)
    ...
    def proc_salva_arquivo(self, p_texto):
        try:
            v_caminho       =   self.VMBRAUNA_CONFIGURACAO['MB_DIRETORIO_LOG'] + "MBLOG" + strftime("%Y%m%d%H%M%S", gmtime()) +".log"
